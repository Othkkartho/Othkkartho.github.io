...
#### 실제 코드
서버에 사용자의 세션이 2개 생성된 것은 맞지만 첫번째 사용자가 서버 자원에 접근하려고 하면 서버는 사용자의 세션이 만료 되었는지 아닌지 확인하는 ConcludeSessionFilter가 체크를 합니다. 만약 세션이 만료된것을 확인하면 사용자의 세션을 실제로 만료시키게 되고, 사용자 세션 만료를 응답을 보내게됩니다.
