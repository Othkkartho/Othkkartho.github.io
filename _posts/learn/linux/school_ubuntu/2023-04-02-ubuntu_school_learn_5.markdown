...
**X윈도우 환경에서 사용자 관리하기**
- 설정 -> 사용자에서 관리 가능
    - 사용자 제거를 누르면 파일 삭제라고 나오는 것을 볼 때 리눅스는 사용자 또한 파일로써 관리하는 것을 알 수 있습니다.
...
| chmod u+x testtxt.txt | testtxt.txt에 대해 소유자에게 실행 권한을 허가하라는 의미|
...
\n
하드 링크 파일 -> inode 1 -> 원본 파일 데이터  
심벌릭 링크 파일 -> inode2 -> 원본 파일 포인터 -> 원본 파일 -> inode1 -> 원본 파일 데이터
...
3. `mv originalfile ../` -> `ls -il` -> `cat hardlink` -> `cat softlink`
